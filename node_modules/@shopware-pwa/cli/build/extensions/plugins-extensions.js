...
module.exports = (toolbox) => {
  ...
  toolbox.fetchPluginsAuthToken = async ({ shopwareEndpoint, username, password } = toolbox.inputParameters) => {
    var _a;
    const normalizedShopwareEndpoint = toolbox.normalizeBaseUrl(shopwareEndpoint);
    let authTokenResponse;
    authTokenResponse = await import_axios.default.post(`${normalizedShopwareEndpoint}/api/oauth/token`, {
    ...
    return (_a = authTokenResponse == null ? void 0 : authTokenResponse.data) == null ? void 0 : _a.access_token;
  };
  ...
  toolbox.fetchPluginsConfig = async ({ config }) => {
    ...
    const url = config.indexOf(endpoint) >= 0 ? config : `${toolbox.normalizeBaseUrl(toolbox.inputParameters.shopwareEndpoint)}/${config}`;
  ...
  toolbox.loadPluginsAssetFile = async ({ asset }) => {
    const endpoint = toolbox.normalizeBaseUrl(toolbox.inputParameters.shopwareEndpoint);
    const fileUrl = asset.indexOf(endpoint) >= 0 ? asset : `${toolbox.normalizeBaseUrl(toolbox.inputParameters.shopwareEndpoint)}/${asset}`;
...
import ${componentNames[index2]} from '${current}'`;
  ...
  toolbox.loadPluginsAssets = async () => {
    ...
    if (!toolbox.inputParameters.username || !toolbox.inputParameters.password) {
      toolbox.print.warning("Plugin settings are not fetched from shopware instance.");
      return;
    }
    try {
      const authToken = await toolbox.fetchPluginsAuthToken(toolbox.inputParameters);
      const buildArtifact = await toolbox.fetchPluginsBuildArtifact(__spreadProps(__spreadValues({}, toolbox.inputParameters), {
        authToken
      }));
      ...
      await toolbox.filesystem.writeAsync(".shopware-pwa/pwa-bundles.json", pluginsConfig);
