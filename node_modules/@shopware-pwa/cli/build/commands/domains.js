...
Synchronize the domain's related config from backend (in order to build a domains.json file)`);
    ...
    if ((!username || !password) && !toolbox.isDefaultDemoData()) {
      toolbox.print.error("Please provide your admin credentials using the --username and --password options or answering the questions.");
      return;
    }
    let authToken;
    try {
    ...
    try {
      ...
      domainsMap = toolbox.domains.prepareDomainsMap(domains, toolbox.normalizeBaseUrl(typeof pwaHost === "string" && pwaHost === "" && toolbox.defaultInitConfig.shopwareEndpoint || pwaHost));
    } catch (error) {
      if (!toolbox.isDefaultDemoData()) {
        console.error(error);
      }
