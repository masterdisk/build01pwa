...
module.exports = {
  ...
  run: async (toolbox) => {
    ...
    if (!languages) {
      toolbox.print.error("Language config does not exist. Pleas run 'yarn shopware-pwa languages' first.");
      return;
    }
    const isoCodes = Object.keys(languages);
    const { username, password } = inputParameters;
    if (!username || !password) {
      toolbox.print.error("Please provide your admin credentials using the --username and --password options.");
      return;
    }
    let authToken;
    try {
    ...
    if (!doExport) {
      toolbox.print.info("Stopping after import (run with --export option to export Snippets to Shopware)");
      return;
    }
